...
export default function Footer() {
  ...
  return (
    <footer className="bg-[#FFF8F0] text-[#131313] mt-20">
      <div className="max-w-screen-2xl mx-auto px-3 sm:px-6 py-12 sm:py-16">
        {/* Main Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8 sm:gap-12 mb-12">
          {/* Brand Section */}
          <div className="lg:col-span-2">
            <Link href="/" className="inline-flex items-center gap-3 group mb-6">
              <div className="w-40 sm:w-48 h-auto shrink-0">
                <Image
                  src="/archnest.png"
                  alt="ArchiNest Logo"
                  width={192}
            ...
            <p className="text-[#727272] text-sm sm:text-base lg:text-lg mb-6 leading-relaxed">
              Build modern, scalable web experiences with cutting-edge technology and innovative solutions.
            </p>
          </div>
          ...
          {footerSections.map((section) => (
            <div key={section.title}>
              <h6 className="font-bold text-[#131313] text-base sm:text-lg mb-4 relative inline-block">
                {section.title}
                <span className="absolute bottom-0 left-0 w-12 h-0.5 bg-gradient-to-r from-[#FF900E] to-[#FFB95C]"></span>
              </h6>
              <ul className="space-y-2 sm:space-y-3">
                {section.links.map((link) => (
                  <li key={link.title}>
                    <Link
                      href={link.href}
                      className="text-[#727272] text-sm sm:text-base hover:text-[#131313] hover:translate-x-1 inline-block transition-all duration-200"
                    >
                      {link.title}
                    </Link>
                  </li>
        ...
        {/* Newsletter Section */}
        <div className="bg-[#FF900E]/10 backdrop-blur-sm border border-[#FF900E]/20 rounded-xl sm:rounded-2xl p-6 sm:p-8 mb-12">
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
            <div className="flex-1 w-full">
              <h6 className="font-bold text-[#131313] text-xl sm:text-2xl mb-2">
                Stay in the loop
              </h6>
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full lg:w-auto lg:min-w-[350px]">
              <Input
                type="email"
                placeholder="Enter your email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                className="flex-1 bg-white/10 border-[#FF900E]/30 text-[#131313] text-sm sm:text-base placeholder:text-[#727272] focus:border-[#FF900E] focus:ring-[#FF900E]/20"
              />
              <Button
                onClick={handleSubscribe}
                className="bg-gradient-to-r from-[#FF900E] to-[#FFB95C] hover:from-[#FFB95C] hover:to-[#FF900E] text-white font-semibold px-6 sm:px-8 text-sm sm:text-base shadow-lg shadow-[#FF900E]/30 hover:shadow-[#FFB95C]/40 transition-all duration-300 whitespace-nowrap"
              >
                Subscribe
              </Button>
            </div>
        ...
        {/* Bottom Bar */}
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
          <span className="text-[#727272] text-xs sm:text-sm text-center sm:text-left">
            &copy; {new Date().getFullYear()} ArchiNest. All rights reserved. Crafted with ❤️
          </span>
          ...
          <div className="flex gap-3 sm:gap-4">
            {socialLinks.map((social) => (
              <Link
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={`p-2.5 sm:p-3 bg-white/5 rounded-lg sm:rounded-xl border border-white/10 ${social.color} hover:border-transparent transition-all duration-300 hover:scale-110 hover:shadow-lg group`}
              >
                <span className="text-[#131313] group-hover:text-white transition-colors font-bold text-xs sm:text-sm flex items-center justify-center">
                  {social.icon}
                </span>
              </Link>
            ))}
